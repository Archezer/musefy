...
This small GUI is the only file a normal user needs to download. The release
...
class DownloadWindow:
    def __init__(self, root: Tk, total_bytes: int) -> None:
        ...
        self.root.title("Musefy — загрузка")
...
def _download_asset(
    ...
    if actual_size != expected_size:
        raise RuntimeError(
            f"Размер {destination.name} не совпадает: "
            f"{actual_size} вместо {expected_size}."
        )
...
def _assemble_installer(
    ...
    if len(assets) == 1:
        ...
        _download_asset(
            window,
            _asset_url(manifest, str(asset["name"])),
            installer_path,
            int(asset["size"]),
            str(asset["sha256"]),
        )
    ...
    # Verify each release part before appending it, then remove the part. This
    # avoids keeping all downloaded parts and the final installer at once.
    for asset in assets:
        part_path = temporary_dir / str(asset["name"])
        _download_asset(
            window,
            _asset_url(manifest, str(asset["name"])),
            part_path,
        ...
        with installer_path.open("ab") as output, part_path.open("rb") as part:
...
def main() -> int:
    ...
    try:
        ...
        try:
            installer_path = _assemble_installer(
                window,
                manifest,
                profile,
                temporary_dir,
