...
Файл `data/models/maest/maest.json` уже входит в репозиторий и содержит названия
519 классов Discogs23. ONNX-файл нужно передать отдельно или скачать из
релиза проекта, когда он будет опубликован.
...
Итоговые жанры фильтруются по `score >= 0.1`. Для каждого жанра также
сохраняются rank и rank-weight. На экран можно выводить первые два жанра, а
остальные оставлять для рекомендательной логики.
