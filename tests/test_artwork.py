...
class FakeArtworkResponse(io.BytesIO):
...
def test_remote_artwork_is_persisted_as_track_cover(
    tmp_path: Path,
    monkeypatch,
) -> None:
    covers_directory = tmp_path / "track_covers"
    ...
    monkeypatch.setattr(
        artwork_module,
        "urlopen",
        lambda _request, timeout: FakeArtworkResponse(
            b"\x89PNG\r\n\x1a\nremote-cover",
            "image/png",
        ),
    )

    cover_path = artwork_module.save_remote_artwork(
